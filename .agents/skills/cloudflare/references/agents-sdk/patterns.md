...
export class TaskAgent extends Agent<Env> {
  ...
  async syncData() {
    ...
    this
      .sql`INSERT INTO cache (data,updated_at) VALUES (${JSON.stringify(data)},${Date.now()})`;
  }
}
```
...
export class EmailAgent extends Agent<Env> {
  async onEmail(email: AgentEmail) {
    ...
    this.connections.forEach((c) =>
      c.send(JSON.stringify({ type: 'new_email', from, subject }))
    );
  }
}
```
...
export class GameAgent extends Agent<Env, GameState> {
  ...
  async onConnect(conn: Connection, ctx: ConnectionContext) {
    ...
    this.setState({
      ...this.state,
      players: [...this.state.players, { id: playerId, score: 0 }],
    });
  }
  async onMessage(conn: Connection, msg: WSMessage) {
    const m = JSON.parse(msg as string);
